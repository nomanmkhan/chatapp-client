    ...
    return (
        <div className={own ? 'message own' : 'message'}>
            <div className='messageTop' >
                <Avatar size={"large"} className='msgImage' style={{ color: '#f9f9f9', backgroundColor: '#4e2261' }}>{own ? readCookie('username').split('')[0].toUpperCase() : friend && friend.username.split('')[0].toUpperCase()}</Avatar>
